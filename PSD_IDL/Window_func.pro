...
for j=0L,Nsim-1,1 do begin
  if ( jt lt Nt) then begin
     ...
     ;;if ( (t_0[jt] - j*bin) ge -(bin*0.001D) ) and ( (t_0[jt] - (j+1)*bin) le -(0.0001D*bin) ) then begin
       ...
       window[j]=1
       ;print,'for window ',j+1,' at time ',t_0[jt], ' is sampled'
       jt++;
     endif else begin 
       window[j]=0
       simx[j]=m
       ;print,'for window ',j+1,' at time ',t_0[jt], ' is *NOT* sampled!'
     endelse 
  endif else begin
     window[j]=0
     simx[j]=m
     ;print,'for window ',j+1,' after time ',t_0[jt], 'NOT sampled!'
  endelse
endfor
