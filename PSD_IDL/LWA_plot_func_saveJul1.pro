...
oploterror,frqc,psdc,errfc,errc,color=Green,errcolor=Green,/nohat,psym=8
...
oplot,cutfreq,10^((psdcutfit[0])+(psdcutfit[1])*alog10(cutfreq)),thick=5,linestyle=3,color=Green

xyouts,[max(frqcr)*0.15,max(frqcr)*0.15],[max(psdcr)*9.,max(psdcr)*9.],'sim cut rebin LC:'+string(psdcrfit[1], format='(f0.3)'),font=0,color=Blue
...
xyouts,[max(frqcr)*0.15,max(frqcr)*0.15],[max(psdcr)*1.,max(psdcr)*1.],'sim cut LC index:'+string(psdcutfit[1], format='(f0.3)'),font=0,color=Green
