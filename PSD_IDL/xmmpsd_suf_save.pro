...
set_plot, 'ps'
device,filename='SuF_'+arg_arr[0]+'.eps', /encapsulated, xsize=8 ,ysize=5,/inches,/color,bits_per_pixel=8
...
;from 0.5 to 1.5 in 0.01 steps
Nbeta=101
beta=(findgen(Nbeta)+50)/100.D
...
xyouts,[max(sufoutstruct.realFreqrebin)*0.1,max(sufoutstruct.realFreqrebin)*0.1],[max(sufoutstruct.realPSDrebin)*0.5,max(sufoutstruct.realPSDrebin)*0.5],'index:'+string(psdfit[1])
