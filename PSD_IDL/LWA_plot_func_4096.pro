...
device,filename='simLCandPSD_beta'+string(beta,format='(f0.2)')+'.eps', /encapsulated, xsize=5 ,ysize=7,/inches,/color,bits_per_pixel=8
...
xyouts,[max(realtime-realtime[0])*0.8,max(realtime-realtime[0])*0.8],[(max(realflux)-min(realflux))*0.95+min(realflux),min(realflux)+(max(realflux)-min(realflux))*0.95],'rebinned LC ',font=0,color=Blue
...
xyouts,[max(realtime-realtime[0])*0.8,max(realtime-realtime[0])*0.8],[(max(realflux)-min(realflux))*0.9+min(realflux),min(realflux)+(max(realflux)-min(realflux))*0.9],'fast sampled LC ',font=0,color=Orange
...
plot,frqc,psdc,psym=8,/xstyle,/xlog,/ystyle,/ylog,xrange=[min(VPadfreq)*0.9,max(VPadfreq)*20],yrange=[min(VPadpsd)*4e-1,max(VPadpsd)*1e1],xtitle='!17Frequency (Hz)',ytitle='!17RMS normalized power',font=0,xtickformat='exponent',ytickformat='exponent'
...
xyouts,[max(frqcr)*0.15,max(frqcr)*0.15],[max(psdcr)*9.,max(psdcr)*9.],'rebinned LC:'+string(psdcrfit[1], format='(f0.3)'),font=0,color=Blue
...
xyouts,[max(frqcr)*0.15,max(frqcr)*0.15],[max(psdcr)*1.,max(psdcr)*1.],'fast sampled LC:'+string(psdcutfit[1], format='(f0.3)'),font=0,color=Orange
