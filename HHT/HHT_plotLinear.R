...
HSname=paste0(filebase,"_HS_linear_Rmed_whiteBack.pdf")
pdf(HSname,width=10,height=8)
HHGramImage(hgram, backcol=c(255,255,255))
dev.off();


hspec <- HHSpectrum(hres,dfreq,scaling = "none")

Hspecname=paste0(filebase,"_Hspec_linear_Rmed.pdf")
pdf(Hspecname,width=10,height=8)
HHSpecPlot(hspec, show.imfs = TRUE,show.total = TRUE,freq.span=freq.span)
#, scaling = "log")
dev.off();

Hspecdat=paste0(filebase,"_Hspec_linear.dat")
