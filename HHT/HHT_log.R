...
filename="data/NightlyVERLC_lc_no_err_sec.dat"
#filename=args[1]
...
dat <- read.table(text=strs,skip=4,header=FALSE)
sig=dat[,2]
tn=dat[,1]
...
hgram <- HHRender(hres, dt, dfreq, time.span = time.span, freq.span = freq.span, scaling = "log", combine.imfs = TRUE, verbose = TRUE)

HSname=paste0(filebase,"_HS_log_R.pdf")
pdf(HSname,width=14,height=10)
HHGramImage(hgram)
dev.off();


hspec <- HHSpectrum(hres,dfreq,scaling = "log")

Hspecname=paste0(filebase,"_Hspec_log_R.pdf")
pdf(Hspecname,width=14,height=10)
HHSpecPlot(hspec, show.imfs = TRUE,show.total = TRUE,freq.span=freq.span)
#, scaling = "log")
dev.off();
